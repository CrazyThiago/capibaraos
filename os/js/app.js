...
/* --- block (orig 6031-10256) --- */
    ...
    const DEFAULT_WALLPAPERS = [
      // Gratis
      { id: 'gta_rain',  name: 'GTA V Rain',     type: 'gif',   price: 0,  url: 'https://raw.githubusercontent.com/alexis2003martinezz-blip/komaos-wallpapers-code/main/Grand%20Theft%20Auto%20V%20Rain%20GIF.gif' },
      { id: 'manga',     name: 'Manga Wallpaper',type: 'gif',   price: 0,  url: 'https://raw.githubusercontent.com/alexis2003martinezz-blip/komaos-wallpapers-code/main/Manga%20Wallpaper%20GIF%20(6).gif' },
    ...
    // Permite sobreescribir wallpapers desde config.js
    const WALLPAPERS = (window.CAPY_CONFIG && window.CAPY_CONFIG.WALLPAPERS) || DEFAULT_WALLPAPERS;
    ...
    function renderWallpaper(w) {
      ...
      if (w.type === 'video') {
        ...
        // fallback si ya está cacheado
        if (el.complete && el.naturalWidth) _clearShim();
      }
      el.className = 'wp-media';
      c.appendChild(el);
      setTimeout(() => shim.remove(), 4000); // safety
    ...
    function openApp(id) {
      ...
      if (id === 'pet') { updatePetUI(); if (typeof applyPetAccessory === 'function') applyPetAccessory(); }
    ...
    function petUpdateExpression(mood) {
      ...
      if (!mouth) return;
      if (mood === 'happy') {
        mouth.setAttribute('d', 'M72 84 Q80 92 88 84');
        if (eyeL)  eyeL.setAttribute('ry', '6.5');
        if (eyeR)  eyeR.setAttribute('ry', '6.5');
        capi && capi.classList.remove('sleep');
      } else if (mood === 'sad') {
        mouth.setAttribute('d', 'M72 89 Q80 85 88 89');
        capi && capi.classList.remove('sleep');
      } else if (mood === 'sleep') {
        mouth.setAttribute('d', 'M76 84 Q80 87 84 84');
        if (eyeL)  eyeL.setAttribute('ry', '1.5');
        if (eyeR)  eyeR.setAttribute('ry', '1.5');
        capi && capi.classList.add('sleep');
        // Add Zzz
        const stage = capi && capi.querySelector('.capy-sparkle-layer');
        if (stage && !stage.querySelector('.sleep-zzz')) {
          stage.innerHTML = '<span class="sleep-zzz">Z</span><span class="sleep-zzz">z</span><span class="sleep-zzz">z</span>';
        }
      } else {
        mouth.setAttribute('d', 'M73 84 Q80 89 87 84');
    ...
    const AVATAR_BASES = ['🦫','🐹','🐻','🐼','🐨','🦊','🐺','🐸','🐧','🦁'];
    const AVATAR_ACCS  = ['','🎩','👑','🌸','⚡','🔥','❄️','🎮','🎸','🌈','🧢','🕶️','🎀','🌊','🍀','⭐','🏆','🦋','🎓','💍','🎐','🔮','🛡️','🍭','🧣','🏄','💐','🫧','🎪','🪽','🎵','💣','🧸','🌺','🍒','🎭'];
    const ACC_LABELS   = ['Ninguno','Sombrero','Corona','Flor','Rayo','Fuego','Nieve','Control','Guitarra','Arcoíris','Gorra','Gafas de sol','Lazo','Ola','Trébol','Estrella','Trofeo','Mariposa','Birrete','Anillo','Campana','Cristal','Escudo','Chupachups','Bufanda','Tabla surf','Ramo','Burbujas','Tienda','Alas','Nota musical','Bomba','Peluche','Flor roja','Cerezas','Máscara'];

    // Pet SVG accessories (rendered on the capybara SVG) — coords for 160x160 viewBox
    const PET_ACC_SVG = {
      '': '',
      '🎩': '<ellipse cx="80" cy="36" rx="20" ry="8" fill="#1a1a2e"/><rect x="58" y="18" width="44" height="20" rx="6" fill="#1a1a2e"/><rect x="62" y="12" width="36" height="14" rx="5" fill="#2d2d44"/>',
      '👑': '<polygon points="52,42 58,20 64,38 70,16 76,36 84,16 90,38 96,20 102,42" fill="#ffd700" stroke="#b8860b" stroke-width="1.2"/><rect x="52" y="40" width="50" height="7" rx="2" fill="#ffd700" stroke="#b8860b" stroke-width="0.6"/><circle cx="64" cy="26" r="2.5" fill="#ff4444"/><circle cx="80" cy="20" r="2.5" fill="#4488ff"/><circle cx="96" cy="26" r="2.5" fill="#44cc44"/>',
      '🌸': '<circle cx="80" cy="32" r="6" fill="#ff6b9d"/><circle cx="73" cy="26" r="5" fill="#ff8fb4"/><circle cx="87" cy="26" r="5" fill="#ff8fb4"/><circle cx="73" cy="38" r="5" fill="#ff8fb4"/><circle cx="87" cy="38" r="5" fill="#ff8fb4"/><circle cx="80" cy="32" r="4" fill="#ffd700"/>',
      '⚡': '<polygon points="80,8 76,28 82,28 78,48 90,22 83,22 88,8" fill="#FFD700" stroke="#FFA500" stroke-width="0.8" opacity="0.9"/>',
      '🔥': '<ellipse cx="80" cy="28" rx="12" ry="16" fill="#ff4500" opacity="0.8"/><ellipse cx="80" cy="32" rx="8" ry="11" fill="#ff8c00" opacity="0.9"/><ellipse cx="80" cy="36" rx="4" ry="6" fill="#ffd700"/>',
      '❄️': '<circle cx="80" cy="30" r="3.5" fill="#87ceeb"/><line x1="80" y1="20" x2="80" y2="40" stroke="#87ceeb" stroke-width="1.8"/><line x1="70" y1="30" x2="90" y2="30" stroke="#87ceeb" stroke-width="1.8"/><line x1="73" y1="23" x2="87" y2="37" stroke="#87ceeb" stroke-width="1.2"/><line x1="87" y1="23" x2="73" y2="37" stroke="#87ceeb" stroke-width="1.2"/>',
      '🎮': '<rect x="62" y="18" width="36" height="24" rx="5" fill="#333" stroke="#555" stroke-width="1.5"/><circle cx="72" cy="28" r="3" fill="#e74c3c"/><circle cx="88" cy="28" r="3" fill="#2ecc71"/><rect x="76" y="34" width="8" height="3" rx="1.5" fill="#555"/>',
      '🎸': '<ellipse cx="90" cy="50" rx="14" ry="10" fill="#D2691E" transform="rotate(-25 90 50)"/><rect x="86" y="24" width="4" height="28" rx="2" fill="#8B4513" transform="rotate(-5 88 38)"/><rect x="84" y="10" width="8" height="18" rx="4" fill="#F5DEB3" transform="rotate(-5 88 19)"/><line x1="86" y1="18" x2="90" y2="18" stroke="#DDD" stroke-width="0.5"/><line x1="86" y1="15" x2="90" y2="15" stroke="#DDD" stroke-width="0.5"/>',
      '🌈': '<path d="M55 10 Q80 -5 105 10" fill="none" stroke="#ff0000" stroke-width="3" opacity="0.7"/><path d="M58 14 Q80 2 102 14" fill="none" stroke="#ff8800" stroke-width="3" opacity="0.7"/><path d="M61 18 Q80 8 99 18" fill="none" stroke="#ffff00" stroke-width="3" opacity="0.7"/><path d="M64 22 Q80 14 96 22" fill="none" stroke="#00cc00" stroke-width="3" opacity="0.7"/><path d="M67 26 Q80 20 93 26" fill="none" stroke="#0088ff" stroke-width="3" opacity="0.7"/><path d="M70 30 Q80 26 90 30" fill="none" stroke="#8800ff" stroke-width="3" opacity="0.7"/>',
      '🧢': '<ellipse cx="80" cy="40" rx="26" ry="6" fill="#2196F3"/><path d="M56 40 Q56 20 80 18 Q104 20 104 40" fill="#2196F3"/><rect x="52" y="38" width="56" height="6" rx="3" fill="#1976D2"/>',
      '🕶️': '<rect x="46" y="56" width="26" height="14" rx="5" fill="#1a1a1a" opacity="0.88"/><rect x="88" y="56" width="26" height="14" rx="5" fill="#1a1a1a" opacity="0.88"/><line x1="72" y1="61" x2="88" y2="61" stroke="#1a1a1a" stroke-width="2.5"/><line x1="46" y1="59" x2="38" y2="52" stroke="#1a1a1a" stroke-width="2"/><line x1="114" y1="59" x2="122" y2="52" stroke="#1a1a1a" stroke-width="2"/>',
      '🎀': '<path d="M75 34 Q68 22 75 18 Q80 22 85 18 Q92 22 85 34 Q80 38 75 34Z" fill="#ff69b4"/><path d="M80 34 L80 42" stroke="#ff1493" stroke-width="2"/>',
      '🌊': '<path d="M52 46 Q60 38 68 46 Q76 54 84 46 Q92 38 100 46 Q108 54 108 46" fill="none" stroke="#00bfff" stroke-width="2.5" opacity="0.7" stroke-linecap="round"/><path d="M56 50 Q64 44 72 50 Q80 56 88 50" fill="none" stroke="#00bfff" stroke-width="1.5" opacity="0.4" stroke-linecap="round"/>',
      '🍀': '<g transform="translate(110,35)"><ellipse cx="0" cy="-8" rx="5" ry="8" fill="#2ecc71" transform="rotate(0)"/><ellipse cx="0" cy="-8" rx="5" ry="8" fill="#27ae60" transform="rotate(72)"/><ellipse cx="0" cy="-8" rx="5" ry="8" fill="#2ecc71" transform="rotate(144)"/><ellipse cx="0" cy="-8" rx="5" ry="8" fill="#27ae60" transform="rotate(216)"/><ellipse cx="0" cy="-8" rx="5" ry="8" fill="#2ecc71" transform="rotate(288)"/><circle cx="0" cy="0" r="3" fill="#f1c40f"/></g>',
      '⭐': '<polygon points="110,20 113,30 124,30 115,36 118,46 110,40 102,46 105,36 96,30 107,30" fill="#FFD700" stroke="#FFA500" stroke-width="0.8"/>',
      '🏆': '<text x="80" y="24" text-anchor="middle" font-size="24">🏆</text>',
      '🦋': '<g transform="translate(118,44) scale(1.3)"><ellipse cx="0" cy="-5" rx="6" ry="8" fill="#e040fb" opacity="0.7"/><ellipse cx="0" cy="5" rx="5" ry="7" fill="#ab47bc" opacity="0.7"/><ellipse cx="-8" cy="-3" rx="5" ry="7" fill="#ce93d8" opacity="0.6" transform="rotate(20 -8 -3)"/><ellipse cx="8" cy="-3" rx="5" ry="7" fill="#ce93d8" opacity="0.6" transform="rotate(-20 8 -3)"/><circle cx="-1" cy="-1" r="1" fill="#1a1a1a"/><circle cx="1" cy="1" r="0.8" fill="#1a1a1a"/></g>',
      '🎓': '<polygon points="58,40 60,28 80,18 100,28 102,40" fill="#1a1a2e"/><rect x="56" y="38" width="48" height="6" rx="1" fill="#1a1a2e"/><line x1="80" y1="18" x2="80" y2="12" stroke="#FFD700" stroke-width="1.5"/><circle cx="80" cy="10" r="2" fill="#FFD700"/>',
      '💍': '<circle cx="80" cy="82" r="6" fill="none" stroke="#FFD700" stroke-width="2.5"/><circle cx="80" cy="77" r="2.5" fill="#4488ff"/>',
      '🎐': '<path d="M80 12 L80 28" stroke="#DAA520" stroke-width="1.5"/><circle cx="80" cy="28" r="4" fill="#FFD700" stroke="#DAA520" stroke-width="1"/><path d="M76 32 Q74 44 70 46" fill="none" stroke="#FFD700" stroke-width="1.5" stroke-linecap="round"/><path d="M84 32 Q86 44 90 46" fill="none" stroke="#FFD700" stroke-width="1.5" stroke-linecap="round"/>',
      '🔮': '<circle cx="80" cy="28" r="10" fill="#7B2FBE" opacity="0.7"/><circle cx="80" cy="28" r="7" fill="#9B59B6" opacity="0.5"/><ellipse cx="77" cy="25" rx="3" ry="2" fill="white" opacity="0.3" transform="rotate(-20 77 25)"/>',
      '🛡️': '<path d="M62 22 L80 14 L98 22 L98 38 Q98 50 80 56 Q62 50 62 38Z" fill="#3498db" stroke="#2980b9" stroke-width="1.5" opacity="0.85"/><path d="M80 22 L80 48" stroke="#fff" stroke-width="1" opacity="0.4"/><path d="M68 32 L92 32" stroke="#fff" stroke-width="1" opacity="0.4"/>',
      '🍭': '<circle cx="80" cy="30" r="7" fill="#ff6b9d"/><rect x="78" y="22" width="4" height="10" rx="2" fill="white" opacity="0.6"/><rect x="78" y="37" width="4" height="12" rx="2" fill="white"/>',
      '🧣': '<path d="M56 48 Q80 56 104 48" fill="none" stroke="#e74c3c" stroke-width="8" stroke-linecap="round" opacity="0.8"/><path d="M56 48 Q80 56 104 48" fill="none" stroke="#c0392b" stroke-width="4" stroke-linecap="round" opacity="0.4"/><path d="M98 48 L104 64" stroke="#e74c3c" stroke-width="7" stroke-linecap="round" opacity="0.7"/>',
      '🏄': '<ellipse cx="90" cy="40" rx="22" ry="5" fill="#F4A460" transform="rotate(-15 90 40)"/><line x1="80" y1="36" x2="80" y2="18" stroke="#8B4513" stroke-width="2"/>',
      '💐': '<circle cx="110" cy="38" r="4" fill="#ff6b9d"/><circle cx="118" cy="32" r="4" fill="#ff8fb4"/><circle cx="122" cy="42" r="4" fill="#ffaacc"/><circle cx="114" cy="46" r="3.5" fill="#ff69b4"/><line x1="114" y1="44" x2="114" y2="58" stroke="#2ecc71" stroke-width="2"/>',
      '🫧': '<circle cx="115" cy="35" r="6" fill="none" stroke="#87ceeb" stroke-width="1" opacity="0.6"/><circle cx="125" cy="28" r="4" fill="none" stroke="#87ceeb" stroke-width="0.8" opacity="0.5"/><circle cx="120" cy="44" r="3" fill="none" stroke="#87ceeb" stroke-width="0.8" opacity="0.4"/><ellipse cx="113" cy="33" rx="1.5" ry="1" fill="white" opacity="0.5"/>',
      '🎪': '<polygon points="80,10 72,28 56,28 68,38 64,54 80,44 96,54 92,38 104,28 88,28" fill="#e74c3c" stroke="#c0392b" stroke-width="1" opacity="0.8"/>',
      '🪽': '<path d="M48 42 Q60 20 70 36 Q74 20 80 36 Q86 20 90 36 Q100 20 112 42" fill="white" opacity="0.6" stroke="#ddd" stroke-width="0.5"/>',
      '🎵': '<ellipse cx="115" cy="36" rx="6" ry="5" fill="none" stroke="#333" stroke-width="1.5" transform="rotate(-15 115 36)"/><line x1="120" y1="32" x2="120" y2="16" stroke="#333" stroke-width="1.5"/>',
      '💣': '<circle cx="80" cy="30" r="10" fill="#2c3e50"/><path d="M76 18 L78 12 L82 12 L84 18" fill="#2c3e50"/><line x1="80" y1="22" x2="80" y2="40" stroke="#e74c3c" stroke-width="1.5" opacity="0.6"/>',
      '🧸': '<ellipse cx="80" cy="28" rx="10" ry="9" fill="#D2B48C"/><circle cx="75" cy="25" r="1.5" fill="#333"/><circle cx="85" cy="25" r="1.5" fill="#333"/><ellipse cx="80" cy="29" rx="3" ry="2" fill="#8B4513"/>',
      '🌺': '<circle cx="80" cy="32" r="7" fill="#ff4444"/><circle cx="73" cy="26" r="5" fill="#ff6666"/><circle cx="87" cy="26" r="5" fill="#ff6666"/><circle cx="73" cy="38" r="5" fill="#ff6666"/><circle cx="87" cy="38" r="5" fill="#ff6666"/><circle cx="80" cy="32" r="4" fill="#FFD700"/>',
      '🍒': '<circle cx="72" cy="34" r="7" fill="#c0392b"/><circle cx="90" cy="34" r="7" fill="#e74c3c"/><path d="M74 28 Q80 14 86 28" fill="none" stroke="#27ae60" stroke-width="1.5"/><ellipse cx="80" cy="16" rx="4" ry="3" fill="#27ae60" transform="rotate(10 80 16)"/>',
      '🎭': '<path d="M60 50 Q80 40 100 50" fill="#FFD700" stroke="#DAA520" stroke-width="1"/><path d="M60 50 Q80 60 100 50" fill="white" stroke="#ccc" stroke-width="1"/><circle cx="72" cy="48" r="2" fill="#333"/><circle cx="88" cy="48" r="2" fill="#333"/>',
    };

    function applyPetAccessory() {
      const acc = getAvatarAcc();
      const mount = document.getElementById('pet-accessory');
      if (!mount) return;
      mount.innerHTML = PET_ACC_SVG[acc] || '';
    }
    ...
    function setAvatarAcc(e) {
      ...
      syncAvatarAll(); refreshAvatarPicker();
      if (typeof applyPetAccessory === 'function') applyPetAccessory();
    }
    ...
    function syncAvatarAll() {
      ...
      // Emoji-only spots (lockscreen capi, auth capi) — NOT the pet, it uses SVG now
    ...
    function renderAvatarPicker() {
      ...
      // acc grid — show all accessories
      const ag = document.getElementById('avatar-acc-grid');
      if (ag) {
        ag.innerHTML = '';
        AVATAR_ACCS.forEach((a, i) => {
          const d = document.createElement('div');
          d.className = 'avatar-option' + (a === getAvatarAcc() ? ' selected' : '');
          d.textContent = a || '❌';
          d.title = ACC_LABELS[i] || '';
          d.onclick = () => setAvatarAcc(a);
          ag.appendChild(d);
        });
      }
    }
    function refreshAvatarPicker() {
      ...
      document.querySelectorAll('#avatar-acc-grid .avatar-option').forEach((d,i) => {
        d.classList.toggle('selected', AVATAR_ACCS[i] === getAvatarAcc());
      });
    }
